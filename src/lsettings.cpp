...
bool LSettings::setIniSetting(const QString &filePath, const QString &name, const QString &value, const qint64 maxLinesCount, const qint64 maxLineLength)
{
    QFile file(filePath);
    ...
    while (!file.atEnd() && l < maxLinesCount) {
        ...
        if (pos == 0 && line.startsWith(name.toUtf8()) && line.contains(QByteArrayLiteral("="))) {
